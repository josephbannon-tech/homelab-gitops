...
Fix: pair the existing scrape-success signal with a probe-based signal that hits the actual service port. Deployed `prometheus-blackbox-exporter` with a `dns_pihole` module sending real DNS queries to `192.168.0.205:53/udp` and asserting `valid_rcodes: [NOERROR]`. New `slo:pihole_dns:burn_rate:*` recording rules over `probe_success{module="dns_pihole"}` plus a critical `PiholeDnsBurnFast` alert. The original `PiholeErrorBudgetBurnFast` stayed in place at warning severity as an exporter-health signal, not a user-facing SLI. Two failure modes, two alerts, two severities, two distinct PromQL expressions. (Postscript, 2026-09: even at warning, a 99.9% SLO on an exporter paged on every self-healing restart; the burn-rate pair was replaced by a single `PiholeExporterWedged` alert that fires only when the liveness probe has demonstrably failed to recover the pod.)
...
**Promtail's varlogs scrape fails noisily once rsyslog is actually installed**
Symptom: months after the journal scrape was working, the Proxmox host got rsyslog installed (to restore a kernel-log forensics window past the 200 MB `journald` cap). `/var/log/syslog` and `/var/log/kern.log` started populating, but Loki returned no `{job="varlogs"}` lines from the host. `journalctl -u promtail` repeated `failed to start tailer ... permission denied filename=/var/log/syslog` every ten seconds.
Root cause: the `varlogs` stanza had always been in the canonical Promtail config, but until rsyslog landed the files didn't exist, so the scrape was a no-op and the permission gap was invisible. Once the files appeared, the `promtail` system user discovered it could not read them: on Debian/Ubuntu both files are mode `0640 root:adm`, and the install procedure only added `promtail` to `systemd-journal`.
Fix: `usermod -a -G adm promtail && systemctl restart promtail`. The install procedure now creates the user in both groups in one step (`usermod -a -G systemd-journal,adm promtail`) so a later rsyslog install does not re-bite.
Lesson: a no-op configuration block can hide a future failure. The varlogs scrape had been carried in the config for fleet consistency on the assumption that "no files, no problem", but the moment the files exist the scrape starts probing real permissions. Audit dormant configuration on its own merits, not on "it hasn't fired yet". Same class as the earlier silent-fail lessons, with a different shape: this one fails noisily instead of silently, which is the easier failure mode to debug but only because the consequence is visible at all.
...
**node_exporter textfile collector drops samples when two `.prom` files declare a conflicting `# TYPE`**
Symptom: a new per-guest vzdump backup heartbeat hook landed alongside the existing rsync-style backup heartbeats. Both wrote `backup_last_success_timestamp_seconds{name="..."}` to `/var/lib/node_exporter/textfile_collector/`. The new file existed on disk, `node_textfile_mtime_seconds` listed it (so the collector had successfully read the file), and `node_textfile_scrape_error` was 0. Yet `curl localhost:9100/metrics | grep ^backup_last_success` returned only the old script's sample; the new one never appeared, and Prometheus had no series to alert on.
Root cause: the existing scripts emitted bare samples with no metadata, so the collector inferred `# TYPE backup_last_success_timestamp_seconds untyped`. The new hook helpfully added `# HELP` and `# TYPE backup_last_success_timestamp_seconds gauge` headers. node_exporter's textfile collector aggregates files per metric name and silently rejects conflicting `# TYPE` declarations; one sample is kept, the other is dropped without surfacing any error. The aggregation comment in the output (`Metric read from <file1>, <file2>`) makes both files look like they contributed, when in fact only one did.
Fix: match the established convention in the directory. If existing scripts omit `# HELP`/`# TYPE`, the new one must too. After dropping the headers, both samples emit on the next scrape and Prometheus picks up the new `name=` value within the minute.
Lesson: a textfile that the collector successfully reads but whose samples are silently rejected presents identically to a healthy one. The structural assumption is that `.prom` files are isolated, but they all merge into a single namespace per metric name, and any disagreement on metadata is resolved by deletion, not warning. Before adding a new script that shares a metric name with existing ones, grep the others (`grep "^# TYPE <metric>" /usr/local/bin/*.sh`) and match the pattern. Same silent-fail class as the colliding-label lesson above and at one more remove than the journal-group lesson, because the failure surface only appears when multiple writers exist.
...
**A liveness probe is a caller too: the probe that provably could not fire**
Symptom: the Pi-hole exporter's scrape sat at exactly the 10 s scrape timeout for 27 hours (`up{job="pihole"}=0`), while the pod stayed `1/1 Running` with zero restarts. An httpGet liveness probe on `/metrics` with a 5 s timeout and `failureThreshold: 6` had been added three months earlier specifically to catch this wedge. Pi-hole itself answered every API call in under a millisecond and the household DNS probe never blinked.
Root cause: traced with `strace -f -tt` and `tcpdump` inside the pod's network namespace. The exporter fetched all seven Pi-hole API responses in 0.18 s, then wrote nothing for 16 s, and answered at the exact moment the *kubelet probe's* collection completed. Upstream v1.2.0's `/metrics` handler collects under a 10 s context and, on timeout, spawns a goroutine to discard the late result from the client's `Status` channel, but then still blocks reading that same channel itself. The channel is one per Pi-hole target, capacity 1, shared by every request. After one slow collection there are two readers racing for every result, so each request returns only when the *next* caller's collection lands. With Prometheus, liveness, and readiness all calling every 30 s, the chain never drains, and whichever waiter is on the channel when a result arrives wins, which means some probe always gets a fast answer and six consecutive failures cannot happen. The probe design was not too loose; it was unreachable by construction. (Upstream: eko/pihole-exporter#332; fix PRs #328/#330 unmerged, maintainer inactive since the release.)
Fix: readiness probe removed (a third caller that gated nothing, since Prometheus scrapes the pod IP via Endpoints), liveness `failureThreshold` 6 to 2 (with two callers a wedged pod answers ~30 s late, fails the 5 s timeout twice, restarts in ~60 s), the burn-rate alert pair on the exporter SLI retired in favour of a single "liveness did not recover it" warning, and the image switched to a maintained fork (`ghcr.io/tomoconnor/pihole-exporter`, per-request result channel, pinned by digest) after diffing all 20 metric names against v1.2.0. Postmortem-grade write-up in the runbook: [pihole-exporter-burn.md](https://github.com/josephbannon-tech/homelab-observability/blob/main/runbooks/alerts/pihole-exporter-burn.md).
Lesson: when a service's failure mode couples requests, every probe you add is another participant in the failure, not an observer of it. Before designing a probe for a wedge, establish whether the wedge is per-request or shared state; a five-minute strace in the pod netns answers that, and would have saved three months of a probe that looked right in the manifest and could never trip. The second lesson is older and sharper: "pod Running, scrape pinned at the timeout, target instantly reachable by hand" is the fingerprint of a bug in the exporter's request handling, not in the thing it monitors. Trace first, restart second.
